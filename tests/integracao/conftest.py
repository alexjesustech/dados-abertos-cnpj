"""Fixtures de integração: SQLite descartável + TestClient da API real.

Cria um banco mínimo no `tmp_path` da sessão com o DDL completo das 11
tabelas + dados sintéticos de 2 empresas (numérica + alfanumérica), 3
estabelecimentos e 5 sócios. O `client` aponta `DB_PATH` pra essa fixture
antes de importar `app.main`, garantindo que o lifespan da API conecte
nela em vez do banco real de 37 GB.
"""
...
def tmp_db_path(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    # Empresa A — numérica, com Simples, matriz + filial, 5 sócios
    conn.execute(
    ...
    # Empresa B — alfanumérica (NT 2025), sem Simples, sem sócios, só matriz
    conn.execute(
        "INSERT INTO empresas VALUES (?,?,?,?,?,?,?)",
