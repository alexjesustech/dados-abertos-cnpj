...
class DatabaseManager:
    ...
    def init_db(self):
        ...
        try:
            ...
            conn.commit()
            self.notifier.log_and_notify("Tabelas do banco de dados criadas/verificadas com sucesso.")
        except Exception as e:
            self.notifier.log_and_notify(f"Erro ao inicializar tabelas do banco de dados: {e}", level=40)
            raise e
        finally:
            conn.close()
    ...
    def import_csv_file(self, csv_path: str, chunk_size: int = 50000):
        ...
        if not table_name:
            self.notifier.log_and_notify(f"Arquivo CSV não correspondente a nenhuma tabela: {path.name}", level=30)
            return

        self.notifier.log_and_notify(f"Importando dados de {path.name} para a tabela '{table_name}'...")
            ...
            with open(csv_path, 'r', encoding='latin-1', errors='ignore') as f:
                ...
                for row in reader:
                    ...
                    if len(buffer) >= chunk_size:
                        ...
                        buffer = []
                        self.notifier.log_and_notify(f"[{table_name}] Ingeridos {total_rows:,} registros...")
    ...
    def import_csv_from_zip(self, zip_path: str, chunk_size: int = 50000) -> bool:
        ...
        if self.is_file_imported(filename):
            self.notifier.log_and_notify(f"Arquivo ZIP '{filename}' já foi importado anteriormente. Pulando.")
            return True
        ...
        self.notifier.log_and_notify(f"Iniciando leitura por streaming do arquivo ZIP: {filename}...")
        ...
        try:
            ...
            with zipfile.ZipFile(zip_path, 'r') as z:
                ...
                csv_files = [
                    ...
                    and (f.endswith(".csv") or any(keyword in f.upper() for keyword in [
                        ".EMPRE", ".ESTABE", ".SOCIOC", ".SIMPLES", ".CNAE", 
                        ".MOTI", ".MUNIC", ".PAIS", ".QUALS", ".NATJU"
                    ]))
                ]
                
                if not csv_files:
                    self.notifier.log_and_notify(f"Nenhum arquivo de dados correspondente encontrado dentro do ZIP: {filename}", level=30)
                    return False
                
                for csv_internal_name in csv_files:
                    table_name = self.get_table_name_from_file(csv_internal_name)
                    if not table_name:
                        self.notifier.log_and_notify(f"Arquivo interno {csv_internal_name} não corresponde a nenhuma tabela mapeada. Pulando.", level=30)
                        continue
                    
                    self.notifier.log_and_notify(f"Importando {csv_internal_name} (dentro de {filename}) para a tabela '{table_name}'...")
            ...
            self.notifier.log_and_notify(f"Erro ao processar streaming do ZIP {filename}: {e}", level=40)
            conn.rollback()
            raise e
        finally:
            conn.close()
